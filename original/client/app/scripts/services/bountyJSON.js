  ...
  return {
    parse: function (src) {
      ...
      function replaceCallExpression(src) {
        ...
        // we won't bother using a $bountyType for this one.
        ...
        src = src.replace(/^\s*(new\s+)?(Date|RegExp)(\b)/, '$1ISO$2$3');
    ...
    stringify: function (value, pretty, autoCollapse) {
        ...
        function createView(key, holder) {
          ...
          switch (typeof value) {
            ...
            case 'object':
              ...
              // This is a serialized bounty type, print it as something awesomer.
              if (Object.hasOwnProperty.call(value, '$bountyType')) {
                switch (value.$bountyType) {
                  case 'ObjectID':
              ...
              for (k in value) {
                if (Object.hasOwnProperty.call(value, k)) {
                  ...
                  if (v) {
                    ...
                    if (isDbRef && k == '$id') {
                      p.setAttribute('data-document-id', bounty.Util.encodeDocumentId(value[k]));
                    }
...
//          var value = holder[key];
//
//          if (angular.isObject(value) && typeof value.toJSON === 'function') {
//            value = value.toJSON(key);
//          }
//
//          // What happens next depends on the value's type.
...
//                switch (value.$bountyType) {
//                  case 'ObjectID':
//                    return c('ObjectID', [quote(value.$value)]);
//
//                  case 'ISODate':
//                    return c('ISODate', [quote(value.$value)]);
//
//                  case 'RegExp':
//                    // we'll render regexp as a literal
//                    // TODO: something might need escaped here?
...
//
//                  case 'BinData':
//                    return c('BinData', [String(value.$value.$subtype), quote(value.$value.$binary)]);
//                }
//              }
//              // Make an array to hold the partial results of stringifying this object value.
//              gap += indent;
...
//                length = value.length;
//                for (i = 0; i < length; i += 1) {
//                  partial[i] = createView(i, value) || 'null';
//                }
//                v = partial.length === 0
//                  ? '[]'
//                  : gap
